...
async function sol2uml() {
    ...
    if (fileFolderAddress.match(/^0x([A-Fa-f0-9]{40})$/)) {
        ...
        const etherscanParser = new etherscanParser_1.default('ZAD4UI2RCXCQTP38EXS3UY2MPHFU5H9KB1');
        umlClasses = await etherscanParser.getUmlClasses(fileFolderAddress);
    }
    else {
        umlClasses = await fileParser_1.parseUmlClassesFromFiles([fileFolderAddress]);
    }
